#!/bin/bash

REMOTE="https://github.com/GrapheneOS/platform_manifest.git"

help() {
    echo -e "${CRB}=========================================================${NC}"
    echo -e " Remote Local Tag: ${CRY}$REMOTE_LOCAL_TAG${NC}"
    echo -e " Remote Latest Tag: ${CRY}$REMOTE_LATEST_TAG${NC}"
    echo -e "${CRB}=========================================================${NC}"
    echo ""
    echo -e "${CRY}Usage:${NC} dtool --repo ${CRG}<command>${NC} [options]"
    echo ""
    echo -e "${CRY}Commands:${NC}"
    echo -e "  ${CRG}init${NC} <number>\t\tInitialize remote repository."
    echo -e "               \t\tRequires a repository/build number."
    echo ""
    echo -e "  ${CRG}update${NC} <number>\tUpdates remote repository."
    echo -e "               \t\tRequires a target update version number."
    echo ""
    echo -e "  ${CRG}sync${NC} [fast/normal]\tSyncing files with remote repository."
    echo -e "               \t\tDefaults to 'fast' if no option is passed."
    echo ""
    echo -e "  ${CRG}reset${NC}\t\t\tHard reset any local changes made to system."
    echo -e "               \t\t${CRY}Warning:${NC} This will wipe all uncommitted local edits."
    echo -e "               \t\t${CRY}Warning:${NC} This will wipe all uncommitted local edits."
    echo ""
}

get_latest_stable_tag() {
    REMOTE_LATEST_TAG=$(git ls-remote --tags --refs "$REMOTE" | awk -F/ '{print $3}' | grep -E '^[0-9]+$' | sort -rn | head -n1)
    save_config_to_data
}

init_remote() {
    echo -e "${CRB}=========================================================${NC}"
    if [ -z "$1" ]; then
        echo -e "Remote Tag: ${CRY}$REMOTE_LATEST_TAG${NC}"
        echo -e "${CRB}=========================================================${NC}"
        repo init -u "$REMOTE" -b "refs/tags/$REMOTE_LATEST_TAG"
    else
        echo -e "Remote Tag: ${CRY}$1${NC} (${CRY}Custom${NC})"
        repo init -u "$REMOTE" -b "refs/tags/$1"
        REMOTE_LOCAL_TAG="$1"
    fi
}

update_remote() {
    echo -e "${CRB}=========================================================${NC}"
    if [ -z "$1" ]; then
        echo -e "Remote Tag: ${CRY}$REMOTE_LATEST_TAG${NC}"
        echo -e "${CRB}=========================================================${NC}"
        repo init -b "refs/tags/$REMOTE_LATEST_TAG"
        REMOTE_LOCAL_TAG="$REMOTE_LATEST_TAG"
        save_config_to_data
    else
        echo -e "Remote Tag: ${CRY}$1${NC} (Custom${NC})"
        repo init -b "refs/tags/$1"
        REMOTE_LOCAL_TAG="$1"
        save_config_to_data
    fi
}

check_for_update() {
    echo -e "${CRB}=========================================================${NC}"
    if [ "$REMOTE_LOCAL_TAG" != "$REMOTE_LATEST_TAG" ]; then
        echo -e " Remote Local Tag:  ${CRY}$REMOTE_LOCAL_TAG{NC} (${CRR}Outdated!${NC})"
        echo -e " Remote Latest Tag: ${CRY}$REMOTE_LATEST_TAG${NC}"
        echo -e " Run \"${CRY}dtool --repo update${NC}\" to get the latest version."
    else
        echo -e " Remote Local Tag: ${CRY}$REMOTE_LOCAL_TAG${NC} (${CRG}Up to date${NC})"
    fi
    echo -e "${CRB}=========================================================${NC}"
}

sync_remote() {
    check_for_update
    if [ "$1" == "normal" ]; then
        repo sync
    else
        repo sync -c --force-sync --optimized-fetch --no-tags --no-clone-bundle --prune -j"$(nproc --all)"
    fi
}

reset_remote() {
    repo forall -c "git reset --hard && git clean -fdx"
}

if ! repo &> /dev/null; then
    echo -e " ${CRY}WARNING${NC} repo is not installed on this device"
    return 10
fi

get_latest_stable_tag

case "$1" in
   "init")
      init_remote "$2"
   ;;
   "update")
      update_remote "$2"
   ;;
   "sync")
      sync_remote "$2"
   ;;
   "reset")
      reset_remote
   ;;
   "check-for-update")
      check_for_update
   ;;
   *)
      help
   ;;
esac